...
/** POST /api/billing/upgrade — paid upgrade to a higher plan while subscribed. */
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      console.warn("[POST /api/billing/upgrade] remember cycle:", err);
    }

    const { result, freshToken } = await withVedmintToken(ctx.userId, (jwt) =>
      upgradeSubscription(jwt, {
        planId,
        billingCycle,
    ...
    if (!result?.payment_url) {
      return NextResponse.json(
        { error: "Upgrade did not return a payment URL" },
        { status: 502 },
      );
    }

...
describe("inviteUrl", () => {
  it("joins path correctly with no trailing slash", () => {
    expect(inviteUrl("abc", "https://wa.vedmint.example")).toBe(
      "https://wa.vedmint.example/join/abc",
    );
  });

  it("tolerates a trailing slash on baseUrl", () => {
    expect(inviteUrl("abc", "https://vedmint.example/")).toBe(
      "https://vedmint.example/join/abc",
    );
  });
